...
class ConsecutiveHolidays with _$ConsecutiveHolidays {
  factory ConsecutiveHolidays({required String title, required List<EventDate> dateList, required DateState state}) =
      _ConsecutiveHolidays;

  ConsecutiveHolidays._();

  factory ConsecutiveHolidays.fromEventDateList(List<EventDate> list) {
    String holidaysString =
        list.where((element) => element.type == EventDateType.holiday).map((e) => e.name).toSet().join(",");
